...
class ActiveLearningData:
    ...
    def __init__(self, dataset: data.Dataset, prev_dataset):
        super().__init__()
        if prev_dataset:
            total = ConcatDataset([prev_dataset, dataset])
            self.dataset = total
        else: 
            total = dataset
            self.dataset = dataset

        print("TOTAL DATASET", len(total), len(dataset))
        self.training_mask = np.full((len(total),), False)
        if prev_dataset: ###include those as part of training
            indices = [idx for idx in range(len(prev_dataset))]
            self.training_mask[indices] = True
        self.pool_mask = np.full((len(total),), True)
        if prev_dataset:
            indices = [idx for idx in range(len(prev_dataset))]
...
def get_balanced_sample_indices(dataset: data.Dataset, num_classes, n_per_digit=2) -> List[int]:
    ...
    for i in range(len(permed_indices)):
        ...
        _, label, _ = dataset[permed_index]
...
def get_sensitive_balanced_sample_indices(dataset: data.Dataset, num_classes, num_sensitives, n_per_digit=2) -> List[int]:
    """Given `target_classes` randomly sample `n_per_digit` for each of the `num_classes` classes."""
    permed_indices = torch.randperm(len(dataset))
    ...
    num_samples_by_class_and_sensitives = {}

    for c in range(num_classes):
        for s in [0.0, 1.0]:
            num_samples_by_class_and_sensitives[(c, s)] = 0
    ...
    for i in range(len(permed_indices)):
        permed_index = int(permed_indices[i])
        _, label, sensitive = dataset[permed_index]
        index, target = permed_index, (int(label), int(sensitive))

        num_target_samples = num_samples_by_class_and_sensitives[target]
        if num_target_samples == n_per_digit:
            continue

        initial_samples.append(index)
        num_samples_by_class_and_sensitives[target] += 1

        if len(initial_samples) == num_classes * num_sensitives * n_per_digit:
            break


    #print(num_samples_by_class_and_sensitives)
